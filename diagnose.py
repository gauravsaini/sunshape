"""
SunShape BCI Diagnostic — one-call analysis for any model.
...
Theory reference:
    The diagnostic computes BCI(Σ_Q, B) for each layer under identity
    vs ProfilePerm, and compares the achievable BCI reduction against
    the rate-dependent noise floor ε(R) ~ 2^{-2R}. See Propositions
    1-3 and Corollary 1 in the SunShape paper.
"""
...
class LayerDiagnostic:
    ...
    def __str__(self) -> str:
        rel_reduction = abs(self.delta_bci) / max(self.bci_identity, 1e-8) * 100
...
def _recommend(
    ...
    if relative_reduction > 0.10 and condition_number > 5.0:
        ...
        return "SunShape-Base (large BCI reduction, alignment reliable)"
    ...
    if relative_reduction > 0.03:
        ...
        if bits_per_dim <= 3.0:
            return "SunShape-Base (moderate BCI reduction, low noise floor helps)"
        return "SunShape-Base (marginal — low noise floor may expose small gains)"

    # Weak: PP removes 1-3% of BCI
    if relative_reduction > 0.01:
        if bits_per_dim >= 3.0:
            return "SunShape-Base or Baseline (small BCI reduction — may help at low noise floor)"
