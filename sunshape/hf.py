...
def infer_attention_capable_layers(model) -> list[dict[str, object]]:
    """Return layer metadata for any layer exposing a diagnosable attention path.

    This is broader than `_infer_supported_kv_layers`, which only returns layers
    that currently plug into the standard HF KV-cache path. Hybrid or recurrent
    layers may appear here even when they are not yet trace-extractable through
    `extract_trace_artifact`.
    """
    from sunshape.diagnose import _find_q_proj

    entries: list[dict[str, object]] = []
    for idx, layer in enumerate(_get_decoder_layers(model)):
        try:
            _mod, desc, head_dim, attn_type = _find_q_proj(layer)
        except Exception:
            continue
        entries.append(
            {
                "layer": int(idx),
                "attention_type": str(attn_type),
                "projection": str(desc),
                "head_dim": int(head_dim),
                "traceable_kv_cache": bool(hasattr(layer, "self_attn")),
            }
        )
    return entries
...
def _apply_rope_pair(model_type: str, query_states: torch.Tensor, key_states: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor):
    if model_type == "gemma4":
        if gemma4_apply_rotary_pos_emb is None:
            raise RuntimeError(
                "Gemma4 rotary helper is unavailable in this transformers build. "
                "Install a transformers version that includes transformers.models.gemma4."
            )
...
def resolve_mode_alias(mode: str) -> str:
    aliases = {
        ...
        "sunshape_mixed": "profileperm_mixed_precision",
        "profileperm_mixed_precision": "profileperm_mixed_precision",
...
def load_model_and_tokenizer(
    ...
    *,
    ...
    hf_kwargs = _hf_pretrained_kwargs(trust_remote_code=trust_remote_code, local_files_only=local_files_only)
    load_kwargs = {
        "dtype": dtype,
        **hf_kwargs,
    }
    ...
    with _hf_offline_context(local_files_only):
        try:
            ...
            ) and Mistral3ForConditionalGeneration is not None:
