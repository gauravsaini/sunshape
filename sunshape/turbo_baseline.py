...
class TurboQuantMSE:
    ...
    def compress(self, x: torch.Tensor) -> TurboQuantized:
        """Return the compressed representation without materializing x_hat."""
    ...
    def attention_scores(
        self,
        query: torch.Tensor,
        quantized: TurboQuantized,
    ) -> torch.Tensor:
        """Exact inner products against compressed TurboQuant-MSE keys.

        This avoids materializing the full dequantized key matrix and matches
        the intended online path more closely than `dequantize(...)+matmul`.
        """
        query = query.to(self.device).float()
        q_rot = query @ self.rotation.T
        y_hat = self.centroids[quantized.indices]
        if quantized.qjl_norms is not None:
            y_hat = y_hat * quantized.qjl_norms.unsqueeze(-1)
        return q_rot @ y_hat.T
